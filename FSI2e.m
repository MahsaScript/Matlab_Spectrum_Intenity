...
for i = 1 : N
    ...
    [Results,LowestError,baseline,BestStart,coeff,xi,yi]=peakfit([xx' yy'],0.8,0,3,2,0,2,0,0,0,0,1); 
    ...
    else if Results(2,2) > Lambda_Tresh
...
STDP2 = std2(IblurSP')
MeanP = mean(IblurSP');
MeanP2 = mean2(IblurSP')
Error = MeanP-MeanP2;
E = std(IblurSP').*ones(size(MeanP));

xxstd = (1:length(MeanP));
figure(25); errorbar(xxstd,MeanP,E);
% ylim([540 570]);
