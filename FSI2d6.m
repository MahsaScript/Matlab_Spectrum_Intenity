...
% addpath('C:\Users\ASUS\Desktop\Code\139912140');
% cd 'C:\Users\ASUS\Desktop\Code\139912140'
...
g1 = 3;                   % Number of Shapes
h1 = 2;

g2 = 2;                   % Number of Shapes
h2 = 1;

g3 = 3;                   % Number of Shapes
h3 = 1;

g4 = 32;
h4 = 3;

% close all; peakfit([xx' yyc'],0.5,0,3,7,0,1);
...
for i = 1 : N
    ...
    [Results,LowestError,baseline,BestStart,coeff,xi,yi]=peakfit([xx' yy'],0.5,0,g1,h1,0,1);
    [numRowsNew,numColsNew] = size(Results);

    if Results(1,2) == 0
            [Results,LowestError,baseline,BestStart,coeff,xi,yi]=peakfit([xx' yy'],0.5,0,3,1,0,1);
            [numRowsNew,numColsNew] = size(Results);
    end
        
    maxAmp = max(yy(158:240));
    ResultsN = Results;

    yit = zeros(1,length(yi));
    for kk= 1: g1
    yit = yit + yi(kk,:);
    end

    [Results_2,LowestError_2,baseline_2,BestStart_2,coeff_2,xi_2,yi_2]=peakfit([xi' yit'],0.5,0,g2,h2,0,1);
    [numRowsNew_2,numColsNew_2] = size(Results_2);


    for h = 1:g2
        if Results_2(h,2) < Lambda_Tresh
            Results_2(h,5) = 0;
        end
        if Results_2(h,4) > 100
            Results_2(h,5) = 0;
        end
        if Results_2(h,2) > 600
            Results_2(h,5) = 0;
        end
    end

    if max(Results_2(:,5)) == 0
            [Results_2,LowestError,baseline,BestStart,coeff,xi,yi]=peakfit([xi' yit'],0.5,0,g4,h4,0,1);
            [numRowsNew,numColsNew] = size(Results_2);
    end
       

    [t1,t2]=max(Results_2(:,5));
        
    valPNew = Results_2(t2,3);
    ppNew = Results_2(t2,2);     
    ...
    yy_No(131:157) = 0; 
    yy_No(131:157) = maxAmp;
    yyc = yy_No;

    [Results_No,LowestError_No,baseline_No,BestStart_No,coeff_No,xi_No2,yi_No2]=peakfit([xx' yyc'],0.5,0,g3,h3,0,1);
    [numRowsNew_No,numColsNew_No] = size(Results_No);


    for h = 1:g3
        if Results_No(h,2) < Lambda_Tresh
            Results_No(h,5) = 0;IXintensity
        end
   ...
   if Results(h,2) == 0
            [Results,LowestError,baseline,BestStart,coeff,xi,yi]=peakfit([xx' yy'],0.5,0,2,1,0,1);
            [numRowsNew,numColsNew] = size(Results);
    end
...
close all
    ...
    yiEstim = zeros(1,length(yi));
    for kk= 1: g1
    yiEstim = yiEstim + yi(kk,:);
    end
...
save spectrum.mat freq yiMeas Results LowestError baseline BestStart coeff xi yi yiEstim
    ...
    yiEstim_No = zeros(1,length(yi_No2));
    for kk= 1: g1
    yiEstim_No = yiEstim_No + yi_No2(kk,:);
    end

freq_No = xx';
yiMeas_No = yyc';
save spectrum_No.mat freq_No yiMeas_No Results_No LowestError_No ...
    baseline_No BestStart_No coeff_No xi_No2 yi_No2 yiEstim_No
...
STDPY = std(IblurSNew); 
STDP2Y = std2(IblurSNew)
MeanPY = mean(IblurSNew); 
MeanP2Y = mean2(IblurSNew)
ErrorY = MeanPY-MeanP2Y; 
EY = std(IblurSNew).*ones(size(MeanPY)); 

xxstdY = (1:length(MeanPY))*Pixelsize;
figure(41); errorbar(xxstdY,MeanPY,EY);
% ylim([540 570]);
xlabel('\mu m');
ylabel('Wavelength [nm]');

save statistY.mat STDPY STDP2Y MeanPY MeanP2Y ErrorY EY xxstdY
% A = load('statist.mat');
...
STDP_NoY = std(IblurSNew_No); 
STDP2_NoY = std2(IblurSNew_No)
MeanP_NoY = mean(IblurSNew_No); 
MeanP2_NoY = mean2(IblurSNew_No)
Error_NoY = MeanP_NoY-MeanP2_NoY; 
E_NoY = std(IblurSNew_No).*ones(size(MeanP_NoY)); 

xxstd_NoY = (1:length(MeanP_NoY))*Pixelsize;
figure(53); errorbar(xxstd_NoY,MeanP_NoY,E_NoY);
% ylim([540 570]);
xlabel('Y [\mum]');
ylabel('Wavelength [nm]');

save statist_NoY.mat STDP_NoY STDP2_NoY MeanP_NoY MeanP2_NoY Error_NoY E_NoY xxstd_NoY
% A = load('statist.mat');
...
% figure; 
% [Results_No,LowestError_No,baseline_No,BestStart_No,coeff_No,xi_No,yi_No]=peakfit([xx' yyc'],0.5,0,g2,h2,0,1);
% [numRowsNew_No,numColsNew_No] = size(Results_No);
